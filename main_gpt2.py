...
class GPTFinetuner(pl.LightningModule):
    ...
    def __init__(self, hparams, train_dataloader, val_dataloader, test_dataloader, orthography):
        ...
        #self.config = hparams
        self.orthography = hparams.orthography
        ...
        self.tokenizer.add_special_tokens({'additional_special_tokens': ['0'],
                                               "pad_token": "<pad>",
                                               "bos_token": "<staroftext>",
                                               "unk_token": "<UNKOWN>"})
        self.tokenizer.add_tokens(["<RESPONSE>: "])
    ...
    def prepare_batch(self, questions: List[str], answers: List[str], train = False) -> List[str]:

        input_dict = self.tokenizer.batch_encode_plus(
            list(questions), padding=True, truncation=False, return_tensors='pt')

        labels = self.tokenizer.batch_encode_plus(
            list(answers), padding=True, truncation=False, return_tensors='pt')

        assert input_dict['input_ids'].shape[1] + labels['input_ids'].shape[1]  < self.hparams.max_seq_length
        #assert labels.shape[1] < self.hparams.max_seq_length
        if train:
          input_ids = torch.cat((input_dict['input_ids'], labels['input_ids']), 1)
          attention_mask = torch.cat((input_dict['attention_mask'], labels['attention_mask']), 1)
          labels = torch.cat((input_dict['input_ids'], labels['input_ids']), 1)
        else: # inference
    ...
    def training_step(self, batch, batch_nb):
        ...
        input_ids, attention_mask, labels = self.prepare_batch(
            questions=questions, answers=correct_answers, train = True)
    ...
    def inference_step(self, batch, batch_nb: int):
        ...
        #print("input_ids shape at infeence ", input_ids.shape)
        
        if self.orthography == "character_fixed":
          batch_outputs = self.model.generate(
              input_ids=input_ids,
              attention_mask=attention_mask,
              do_sample=False,
              max_new_tokens=self.hparams.max_digits_test + 1,
              pad_token_id = 50257)
          ...
          batch_outputs = self.model.generate(
              ...
              attention_mask=attention_mask,
              do_sample=False,
              max_new_tokens=self.hparams.max_digits_test + 2,
              pad_token_id = 50257) 
        batch_output = batch_outputs[:,input_ids.shape[1]:]
        ...
        if batch_nb & (batch_nb - 1) == 0:
            ...
            print('\nPredicted:', predicted_answers[0].encode('utf-8'))
            print('\nPredictedoutput:', predicted_outputs[0].encode('utf-8'))
    ...
    def on_validation_epoch_end(self):
        ...
        self.outputs.clear()
        self.log("val_exact_match", exact_match, on_epoch=True, prog_bar=True, logger=True)
    ...
    def on_test_epoch_end(self):
        ...
        output['progress_bar'] = metrics
        self.outputs.clear()
        self.log("test_exact_match", exact_match, on_epoch=True, prog_bar=True, logger=True)
        return output
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        self.max_digits -= 1
        if self.orthography == 'character_fixed':
          return f'{first_term} {operation_term} {second_term} = <RESPONSE>: ', f'{answer}'
        else:
          return f'$ {first_term} {operation_term} {second_term} = <RESPONSE>: ', f'{answer} $'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train and evalute GPT2 on arithmetic problems.')
    parser.add_argument('--default_root_dir', type=str, required=True, help='Path to save checkpoint and results.')
    ...
    parser.add_argument("--accumulate_grad_batches", default=32, type=int, help="Accumulates gradients over k batches before stepping the optimizer. Default: 1")
    parser.add_argument("--check_val_every_n_epoch", default=1, type=int, help="Perform a validation loop every after every N training epochs. If None, validation will be done solely based on the number of training batches, requiring val_check_interval to be an integer value. Default: 1")
    parser.add_argument("--max_epochs", default=1, type=int, help="Stop training once this number of epochs is reached. Disabled by default (None). If both max_epochs and max_steps are not specified, defaults to max_epochs = 1000. To enable infinite training, set max_epochs = -1")
    parser.add_argument("--precision", default=32, type=int, help="Number of CPU workers for loading data.")
    parser.add_argument("--gradient_clip_val", default=1.0, type=float, help="The value at which to clip gradients. Passing gradient_clip_val=None disables gradient clipping. If using Automatic Mixed Precision (AMP), the gradients will be unscaled before. Default: None.")
    parser.add_argument("--accelerator", default='gpu', type=str, help='Supports passing different accelerator types (“cpu”, “gpu”, “tpu”, “ipu”, “hpu”, “mps”, “auto”) as well as custom accelerator instances.')
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_weights_only=False, every_n_epochs=args.check_val_every_n_epoch)

    #trainer = Trainer.from_argparse_args(args, callbacks=checkpoint_callback)
    trainer_args = vars(args).copy()
    del trainer_args['model_name_or_path']
    del trainer_args['operation']
    del trainer_args['orthography']
    del trainer_args['invert_question']
    del trainer_args['invert_answer']
    del trainer_args['balance_train']
    del trainer_args['balance_val']
    del trainer_args['balance_test']
    del trainer_args['min_digits_train']
    del trainer_args['min_digits_test']
    del trainer_args['max_digits_train']
    del trainer_args['max_digits_test']
    del trainer_args['base_number']
    del trainer_args['seed']
    del trainer_args['train_size']
    del trainer_args['val_size']
    del trainer_args['test_size']
    del trainer_args['max_seq_length']
    del trainer_args['train_batch_size']
    del trainer_args['val_batch_size']
    del trainer_args['optimizer']
    del trainer_args['lr']
    del trainer_args['weight_decay']
    del trainer_args['scheduler']
    del trainer_args['gamma']
    del trainer_args['step_size']
    del trainer_args['t_0']
    del trainer_args['t_mult']
    del trainer_args['num_workers']
    
    print("trainer_args: ", trainer_args)
    trainer = Trainer(**trainer_args, callbacks=checkpoint_callback)
    model = GPTFinetuner(hparams=args,
    ...
    checkpoint_path = glob.glob(os.path.join(args.default_root_dir, '*.ckpt'))[0]
    model = GPTFinetuner.load_from_checkpoint(checkpoint_path,
                                             train_dataloader=train_dataloader,
                                             val_dataloader=val_dataloader,
                                             test_dataloader=test_dataloader,
                                             orthography=args.orthography)
    ...
    with open(os.path.join(args.default_root_dir, 'results.json'), 'w') as fout:
